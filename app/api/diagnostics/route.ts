  ...
  const data = await withScope({ accountId: account.id }, async (client) => {
    ...
    return {
      ...
      comoLeerlo: {
        ...
        granularidadProbada:
          "dailyWindowTest: si allDiffer=true y matchesBudget=false, confirma que campaigns/search da gasto real por día. Si matchesBudget=true (como pasó con la cuenta de prueba: costA y costB dieron EXACTO el presupuesto configurado), es mala señal — ese campo devuelve el presupuesto, no el gasto real de ese día puntual, así que no sirve para repartir por día. itemLevelAttempts: las tres variantes de .../product_ads/ads/search dieron ok:true — el endpoint EXISTE. itemMetricsCheck valida si esos datos son gasto real por publicación o el mismo problema a otro nivel: itemsWithOtherCampaignId > 0 significa que el filtro no filtra de verdad (devuelve ítems de otras campañas); allItemsSameCost=true o anyItemCostDiffersByDay=false es la misma mala señal que matchesBudget, pero por ítem. Solo si el filtro funciona Y los costos varían de verdad entre ítems y entre días conviene construir la integración real sobre esto.",
        conEnvioEnCero:
