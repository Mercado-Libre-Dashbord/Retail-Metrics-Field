  ...
  const data = await withScope({ accountId: account.id }, async (client) => {
    ...
    return {
      ...
      comoLeerlo: {
        ...
        granularidadProbada:
          "dailyWindowTest: si allDiffer=true (costA, costB y costC — 3, 6 y 25 días atrás — los tres distintos entre sí) y matchesBudget=false (ninguno coincide con el presupuesto configurado de la campaña), confirma que se puede pedir el gasto por día achicando la ventana de fecha, sin ningún endpoint nuevo. Si matchesBudget=true, mala señal: el campo puede estar devolviendo el presupuesto configurado, no el gasto real de ese día puntual. itemLevelAttempts: tres rutas SIN CONFIRMAR para costo por publicación puntual — status 404 en las tres es la señal más fuerte de que esa ruta no existe tal cual; cualquier otra cosa (ok:true, o un status distinto) es una pista real para investigar más. Mandá el bloque completo, no solo el resumen.",
        conEnvioEnCero:
