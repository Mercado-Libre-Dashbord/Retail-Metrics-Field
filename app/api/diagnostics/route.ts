  ...
  const data = await withScope({ accountId: account.id }, async (client) => {
    ...
    return {
      ...
      comoLeerlo: {
        ...
        granularidadProbada:
          "dailyWindowTest: si allDiffer=true (costA, costB y costC — 3, 6 y 25 días atrás — los tres distintos entre sí) y matchesBudget=false (ninguno coincide con el presupuesto configurado de la campaña), confirma que se puede pedir el gasto por día achicando la ventana de fecha, sin ningún endpoint nuevo. Si matchesBudget=true, mala señal: el campo puede estar devolviendo el presupuesto configurado, no el gasto real de ese día puntual. itemLevelAttempts: ahora prueba .../product_ads/ads/search (plano, filtrado por campaign_id — no anidado bajo /campaigns/{id}/, que fue lo que dio 404 en la corrida anterior) con las dos sintaxis de filtro documentadas más el plural campaign_ids. Si alguna da ok:true, ese es el costo por publicación puntual — mandá el bloque completo (path que respondió + sampleKeys) para construir sobre eso. 404 en las tres sigue siendo la señal más fuerte de que ninguna de esas tres formas existe tal cual para esta cuenta.",
        conEnvioEnCero:
